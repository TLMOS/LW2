...
void TestDequeCreation(std::string prefix, bool silent)
{
	if (!silent)
		std::cout << prefix << "Creation: ";

	Deque<int>* deque = new Deque<int>(SequenceType::None);
	assert(deque->GetSequenceType() == (int)Deque<int>::kDefaultSequenceType);
	delete deque;

	deque = new Deque<int>(SequenceType::Array);
	assert(deque->GetSequenceType() == (int)SequenceType::Array);
	delete deque;

	deque = new Deque<int>(SequenceType::List);
	assert(deque->GetSequenceType() == (int)SequenceType::List);
	delete deque;

	int arr[5] = { 1, 2, 3, 4, 5 };
	Sequence<int>* sequence = new ArraySequence<int>(arr, 5);
	deque = new Deque<int>(*sequence, SequenceType::None);
	assert(deque->GetSequenceType() == (int)Deque<int>::kDefaultSequenceType);
	delete deque;

	deque = new Deque<int>(*sequence, SequenceType::List);
	assert(deque->GetSequenceType() == (int)SequenceType::List);
	delete deque;

	deque = new Deque<int>(*sequence, SequenceType::Array);
	assert(deque->GetSequenceType() == (int)SequenceType::Array);
	delete deque;
	delete sequence;

	Deque<int>* deque_1 = new Deque<int>(SequenceType::Array);
	Deque<int>* deque_2 = new Deque<int>(*deque_1, SequenceType::Same);
	assert(deque_2->GetSequenceType() == (int)SequenceType::Array);
	delete deque_1;
	delete deque_2;

	deque_1 = new Deque<int>(SequenceType::List);
	deque_2 = new Deque<int>(*deque_1, SequenceType::Same);
	assert(deque_2->GetSequenceType() == (int)SequenceType::List);
	delete deque_1;
	delete deque_2;
...
void TestDequePopPush(std::string prefix, bool silent)
{
	if (!silent)
...
void TestDequeSort(std::string prefix, bool silent)
{
	if (!silent)
...
void TestDequeMap(std::string prefix, bool silent)
{
	if (!silent)
		std::cout << prefix << "Map: ";
...
void TestDequeWhere(std::string prefix, bool silent)
{
	if (!silent)
...
void TestDequeReduce(std::string prefix, bool silent)
{
	if (!silent)
...
void TestDequeSequenceMethods(std::string prefix, bool silent)
{
	if (!silent)
		std::cout << prefix << "Sequence methods memory check: ";

	float items[5] = { 1, 2, 3, 4, 5 };
	Deque<float>* deque_1 = new Deque<float>(items, 5);
	Deque<float>* deque_2 = new Deque<float>(items, 5);
	Deque<float>* deque_3 = deque_1->Concat(deque_2);
	delete(deque_2);
	delete(deque_3);

	deque_2 = deque_1->GetSubsequence(1, 2);
	delete(deque_1);
	delete(deque_2);
...
void TestDequeAll(std::string prefix, bool silent)
{
	if (!silent)
		std::cout << prefix << "Deque tests:\n";
	prefix += "----";

	TestDequeCreation(prefix, silent);
	TestDequePopPush(prefix, silent);
	TestDequeSort(prefix, silent);
	TestDequeMap(prefix, silent);
	TestDequeWhere(prefix, silent);
	TestDequeReduce(prefix, silent);
	TestDequeSequenceMethods(prefix, silent);
}
...
void TestAll(std::string prefix, bool silent)
{
	...
	TestSequenceAll(prefix, silent);
	TestDequeAll(prefix, silent);
}
